...
manifest_original_path="$(python - "$file_metadata_path" <<'PY'
import json
import sys
...
manifest_archived_path="$(python - "$file_metadata_path" <<'PY'
import json
import sys
...
restore_status="$(python - "$file_metadata_path" <<'PY'
import json
import sys
...
dir_archived_path="$(printf '%s' "$archive_dir_json" | python -c 'import json,sys; print(json.load(sys.stdin)["archived_path"])')"
dir_metadata_path="$(printf '%s' "$archive_dir_json" | python -c 'import json,sys; print(json.load(sys.stdin)["metadata_path"])')"
[ ! -e "$dir_path" ]
[ -d "$dir_archived_path" ]
[ -f "$dir_metadata_path" ]
...
orphan_metadata_path="$(printf '%s' "$orphan_json" | python -c 'import json,sys; print(json.load(sys.stdin)["metadata_path"])')"
orphan_archived_path="$(printf '%s' "$orphan_json" | python -c 'import json,sys; print(json.load(sys.stdin)["archived_path"])')"
...
broken_archived_path="$(printf '%s' "$broken_json" | python -c 'import json,sys; print(json.load(sys.stdin)["archived_path"])')"
broken_metadata_path="$(printf '%s' "$broken_json" | python -c 'import json,sys; print(json.load(sys.stdin)["metadata_path"])')"
[ ! -L "$broken_link" ]
[ -L "$broken_archived_path" ]
[ -f "$broken_metadata_path" ]

broken_kind="$(python - "$broken_metadata_path" <<'PY'
import json
import sys
